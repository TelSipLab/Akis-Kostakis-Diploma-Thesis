...
# rollEKF = np.loadtxt('Results/predicted_roll_ekf.txt')  # Will be added later
...
# rollEKF_sample = rollEKF[start_idx:end_idx]  # Will be added later
...
# rmse_ekf = np.sqrt(np.mean(error_ekf**2))  # Will be added later
...
axes[0].plot(time_indices, rollMahony_sample, 'r-.', linewidth=1.5, label='Mahony')
# axes[0].plot(time_indices, rollEKF_sample, 'g-.', linewidth=1.5, label='EKF')  # Will be added later
...
axes[1].plot(time_indices, abs_error_complementary, 'b-', linewidth=1.5, label=f'Complementary Error - {rmse_complementary:.3f} deg', alpha=0.7)
axes[1].plot(time_indices, abs_error_mahony, 'r-', linewidth=1.5, label=f'Mahony Error - {rmse_mahony:.3f} deg', alpha=0.7)
# axes[1].plot(time_indices, abs_error_ekf, 'g-', linewidth=1.5, label=f'EKF Error - {rmse_ekf:.3f} deg', alpha=0.7)  # Will be added later
...
# print(f"  EKF RMSE: {np.sqrt(np.mean(error_ekf[high_dynamics_mask]**2)):.4f} deg")  # Will be added later
# print(f"  EKF advantage: {rmse_comp_high - np.sqrt(np.mean(error_ekf[high_dynamics_mask]**2)):.4f} deg")
...
# print(f"  EKF RMSE: {np.sqrt(np.mean(error_ekf[low_dynamics_mask]**2)):.4f} deg")  # Will be added later
# print(f"  EKF advantage: {rmse_comp_low - np.sqrt(np.mean(error_ekf[low_dynamics_mask]**2)):.4f} deg")
