#ifndef LSTMNETWORKNOATTENTION_HPP
#define LSTMNETWORKNOATTENTION_HPP
...
public:
    LSTMNetwork(int inputSize, int hiddenStateSize, int outputSize, double dropoutRate = 0.2) {
        ...
        // 2. Dropout for regularization (active only during training)
    ...
    std::string describe() const {
        ...
        ss << "Input(" << inputSize_ << ") -> LSTM(" << hiddenStateSize_ << ") -> Dropout -> FC(" << outputSize_ << ")";
        return ss.str();
    }
    ...
    Tensor forward(Tensor X) {
        ...
        auto out = std::get<0>(lstmOutput); // [batch, seq_len, hidden]

        // Step 2: Take last timestep output
        out = out.select(1, out.size(1) - 1); // [batch, hidden]

        // Step 3: Dropout before FC
        out = dropout->forward(out);

        // Step 4: FC output
        out = fc->forward(out); // [batch, output_size]
        return out;
    }
...
#endif // LSTMNETWORKNOATTENTION_HPP
