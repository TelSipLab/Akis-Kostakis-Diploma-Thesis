...
**Status:** Planning phase - see `MDFiles/RNN_Hybrid_Architecture.md`

**Approach:** Hybrid model combining classical filters (EKF/Mahony) with LSTM for improved accuracy.

**Implementation Language:** C++ using LibTorch (PyTorch C++ API)

### Planned RNN Input/Output (from email thread):

**Input at timestep t_k:**
- Roll angle (from classical filter)
- Pitch angle (from classical filter)
- Angular velocities: ωx, ωy, ωz (from gyroscope)
- Torques: τx, τy, τz (control inputs) - **NOTE: Current dataset lacks torque data**

**Output at timestep t_{k+1}:**
- Predicted roll and pitch angles

**Critical Open Questions:**
1. Torque data availability - current Data/ folder only has gyro, accel, angles
2. Should accelerometer data (ax, ay, az) be included in input features?
3. RNN output: angles only, or angles + angular velocities?
4. Sequence window size: single timestep or lookback window (10-50 steps recommended)
